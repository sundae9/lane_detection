...
void Time_record(cv::TickMeter &tm, Video_info &video_info, int idx) {
    ...
    // 최대값 갱신
    if (video_info.time_info.max_time[idx] < cur_time) {
        video_info.time_info.max_time[idx] = cur_time;
        video_info.frame_info.max_frame[idx] = video_info.total_frame;
    }
    // 최솟값 갱신
    if (video_info.time_info.min_time[idx] > cur_time) {
        video_info.time_info.min_time[idx] = cur_time;
        video_info.frame_info.min_frame[idx] = video_info.total_frame;
    }
...
void Print_info(Video_info &video_info, int idx) {
    ...
    std::cout << video_info.time_info.min_time[idx] << "," << video_info.frame_info.min_frame[idx] << ",";  // 최소 정보
    std::cout << video_info.time_info.max_time[idx] << "," << video_info.frame_info.max_frame[idx] << ",";  // 최대 정보
    std::cout << video_info.time_info.total_time[idx] / (video_info.total_frame - 1) << ",\n";              // 평균 정보
}
 ...
 */
void Print_info_all(Video_info &video_info, int cnt) {
    for (int i = 0; i < cnt; i++) {
        std::cout << i << ",";                                                                              // 프로세스 번호
        std::cout << video_info.time_info.min_time[i] << "," << video_info.frame_info.min_frame[i] << ",";  // 최소 정보
        std::cout << video_info.time_info.max_time[i] << "," << video_info.frame_info.max_frame[i] << ",";  // 최대 정보
        std::cout << video_info.time_info.total_time[i] / (video_info.total_frame - 1) << ",\n";            // 평균 정보
    }
}
